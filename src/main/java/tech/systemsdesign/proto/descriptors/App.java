...
public class App {
    ...
    private static void processProtobufDescriptorSet(File descriptorFile) {
        ...
        try (FileInputStream fis = new FileInputStream(descriptorFile)) {
            ...
            for (FileDescriptorProto fileDescriptorProto : fileDescriptorSet.getFileList()) {
                ...
                fileDescriptorProto.getMessageTypeList().forEach(messageDescriptor -> printMessages(messageDescriptor, 1));
    ...
    private static void printMessages(DescriptorProto messageDescriptor, int indent) {
        ...
        if (messageDescriptor.getFieldCount() > 0) {
            ...
            messageDescriptor.getFieldList().forEach(fieldDescriptorProto -> {
                String field = fieldDescriptorProto.getType() + " " + fieldDescriptorProto.getName() + " = " + fieldDescriptorProto.getNumber() + ";";
                log(field, indent + 1);
            });
            log("}", indent);
        } else {
    ...
    private static void printServiceMethod(MethodDescriptorProto method, int indent) {
        log(method.getName()+"("+method.getInputType()+") returns ("+method.getOutputType()+")", indent);
    }

    // ********** Helper methods for loading descriptor files *********

    private static List<File> getDescriptorFilesList(String descriptorDir, String descriptorFileSuffix) {
        ...
        if (files.size() == 0) {
            log("Error: No files found in directory [" + descriptorDir + "] with suffix [" + descriptorFileSuffix + "]");
            System.exit(2);
        }
        return files;
    }
